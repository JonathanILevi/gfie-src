...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FComp.MakeWarning('', ewCustomWarning, 'LoadExec failed (with preprocessor)');
        Result := False;
      end else
        Result := True;
    end else Result := False;
  ...
  begin
    ...
    begin
      ...
      begin
        FComp.MakeWarning('', ewCustomWarning, 'LoadExec failed');
        Result := False;
      end else
        Result := True;
    end else Result := False;
...
var
  s: tbtstring;
begin
  if (not FComp.GetOutput(s)) then
  begin
    FComp.MakeWarning('', ewCustomWarning, 'FComp.GetOutput failed');
    Result := False;
    exit;
  end;
  if (not FExec.LoadData(s)) then
  begin
    FComp.MakeWarning('', ewCustomWarning, 'FExec.LoadData failed');
    Result := False;
    exit;
  end;
