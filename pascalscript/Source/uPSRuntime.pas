...
interface
uses
  Dialogs, SysUtils, uPSUtils{$IFDEF DELPHI6UP}, variants{$ENDIF}{$IFDEF MACOS},uPSCMac{$ELSE}{$IFNDEF PS_NOIDISPATCH}{$IFDEF DELPHI3UP}, ActiveX, Windows{$ELSE}, Ole2{$ENDIF}{$ENDIF}{$ENDIF};
...
{$ENDIF}
  ...
  // Returns -1 if successful, and an error code (line number) if not
  function LoadProcs: integer;
  var
    Rec: TPSProc;
    n: tbtString;
    b: Byte;
  ...
  begin
    ...
    for l := 0 to HDR.ProcCount - 1 do begin
      ...
      begin
        ...
        //ShowMessage(TPSExternalProcRec(Curr).Name+' '+TPSExternalProcRec(Curr).Decl+' imported');
      end else begin
...
begin
  ...
  if iCode >= 0 then
  begin
    ShowMessage('LoadProcs failed at line '+IntToStr(iCode));
    Clear;
    exit;
  end;
  if not LoadVars then
  begin
    ShowMessage('LoadVars failed');
    Clear;
    exit;
  end;
    ...
    case desttype.BaseType of
      ...
      btS32:
        ...
        {$IFNDEF PS_NOWIDESTRING}  btWideChar: tbts32(Dest^) := Ord(tbtwidechar(Src^));{$ENDIF}
            ...
            // nx change start - allow assignment of class, but only if nil
            btClass:
              begin
                // only allow nil assignment
                tmp := TObject(TbtU32(Src^));
                if tmp = nil then
                  TObject(Dest^) := tmp
                else
                  raise Exception.Create(RPS_TypeMismatch);
