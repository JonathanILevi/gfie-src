...
interface
  uses uPSUtils, SysUtils, StrUtils, ParserUtils, Classes, IniFiles;
...
(*----------------------------------------------------------------------------*)
  ...
  private
    ...
    FClassInheritances: TList; // TODO free this and its children
    procedure AddClassInheritance(const classParent, className: string);
    function GetChildClass(const className: string): string;
...
begin
  inherited create;
  FClassInheritances := TList.Create;
  FUnitPrefix := 'uPS';
...
procedure TUnitParser.AddClassInheritance(const classParent, className: string);
var
  ci: TClassInheritance;
begin
  ci := TClassInheritance.Create;
  ci.FClassParent := classParent;
  ci.FClassName := className;
  FClassInheritances.Add(ci);
end;

function TUnitParser.GetChildClass(const className: string): string;
var
  i: integer;
  ci: TClassInheritance;
begin
  Result := '';
  for i := 0 to FClassInheritances.Count - 1 do
  begin
    ci := TClassInheritance(FClassInheritances[i]);
    if ci.FClassParent = className then
    begin
      Result := ci.FClassName;
      Exit;
    end;
  end;
end;
...
procedure TUnitParser.FinishParseSingleUnit;
  ...
  var
    ...
    s: string;
  begin
    // TODO this is just a hack, provide a proper solution
    {$ifndef WINDOWS}
    if List.Count > 0 then
    for i := List.Count - 1 downto 0 do
    begin
      s := LowerCase(List[i]);
      if (s = 'windows') or (s = 'messages') or (s = 'ddeserver') or (s = 'fileassoc') then
        List.Delete(i);
    end;
    {$endif}
...
begin
  ...
  // generate the actual string
    ...
    NextToken; 
  until (not (TokenID in [CSTI_Period, CSTI_Identifier, CSTI_Integer, CSTI_HexInt, CSTI_Real, CSTI_String, CSTI_Char,
    CSTI_Equal, CSTI_NotEqual, CSTI_Greater, CSTI_GreaterEqual,
      ...
      CSTII_xor, CSTII_Chr, CSTII_Ord, CSTI_OpenRound, CSTI_CloseRound])) and (((BlkBracketCount=0)and(BracketCount=0)) or
      not (TokenID in [CSTI_COmma,CSTI_Colon,CSTI_SemiColon, CSTI_OpenBlock, CSTI_CloseBlock ])) ;
...
var
  ConstName, ConstType, ConstValue, Expression, msg: string;
  l: Longint;
begin
  Match(CSTII_const);
  repeat
   ...
   // Hack: We cannot succesfully parse this, but that doesn't mean we should stop.
      ...
      begin
        ...
        if (e <> nil) then
          msg := e.Message;
//        Writeln('Warning, could not parse const: ' + msg); // this crashes...
        l := 0;
...
// check if we are a method pointer
    ...
    // check for calling conversion
            ...
            begin
              if not (IsMethod in Options) then
                RaiseError('Message directive only applies to methods (' + OwnerClass +ProcName + ')', TokenRow, TokenCol);
              NextToken;
              Match(CSTI_Identifier);
              Match(CSTI_Semicolon);
            end else
...
//                  Writeln('Overloading isnt supported. Remapping of name required '+OwnerClass +Decl); // this crashes
...
//                  Writeln('New Name :''' + ProcName + '''');
                  ...
                  begin
                    ...
                    If (IsMethod in Options) then
                      ...
                      Add('Begin '+s+self.UnitName + '.' + OldProcName +ParamStr+ '; END;');
                  end;
                end;
                NextToken;
                Match(CSTI_Semicolon);
...
var
  ...
  var
    decl, ProcName, callingConvention, PProcname, childClass: string;
        ...
        begin
          ...
          begin
            fCurrentRTProc.Add('    RegisterVirtualAbstractMethod('+aClassname +
              '_Impl, @'+aClassname+'_Impl.' + ProcName + ', ''' + ProcName + ''');');
          end
          else
        ...
        CSTI_Semicolon:
          begin // A property is being introduced that is present in the parent object
            ...
            begin
              ...
              decl := StringReplace(ParamTypes.Text, #13, '', [rfreplaceAll]);
              decl := StringReplace(decl, #10, ' ', [rfreplaceAll]);
              decl := trim(decl);
...
//  NextToken;  the semicolon is removed by the caller
  ...
  begin
    ...
    AddClassInheritance(aClassParent, aClassname);
...
begin
  ...
  case aTokenID of
    ...
    CSTI_Identifier: // simple type by name  (MyInt = Integer)
      begin
        Match(CSTI_Identifier);
        if IfMatch(CSTI_Less) then // generic, not supported
        begin
          TypeDescriptor := '';
          skipToSemicolon;
          CheckSemiColon:=false;
        end else
        if IfMatch(CSTI_OpenBlock) then // string[...], not supported
        begin
          TypeDescriptor := '';
          skipToSemicolon;
          CheckSemiColon:=false;
        end else
        if IfMatch(CSTI_TwoDots) then // 12..32, not supported
        begin
          TypeDescriptor := '';
          skipToSemicolon;
          CheckSemiColon:=false;
...
//        Writeln('Pointers are not supported, this declaration will fail. At position :' + inttostr(TokenRow) + ':' + inttostr(TokenCol)); // this crashes
...
//    TypeDescriptor := TypeDescriptor + 'type';
    ...
    CSTII_record: // records (rec = record a : integer; end;)
      begin
        ...
        if (TokenID = CSTI_Identifier) and (Token = 'HELPER') then
        begin
          NextToken;
          Match(CSTII_for);
          Match(CSTI_Identifier);
        ...
        while not IfMatch(CSTII_end) do NextToken;
      end;
...
//    RaiseError('Sets are not supported',TokenPos);
  ...
  //            Match(CSTI_Period, '..');
            ...
            if IfMatch(CSTI_Comma) then
              TypeDescriptor := TypeDescriptor + ', ';
          until IfMatch(CSTI_CloseBlock);
          TypeDescriptor := TypeDescriptor + '] ';
...
begin {ParseTypes}
  ...
  repeat
    ...
    if IfMatch(CSTI_Less) then // generic
    begin
      // not supported, skip
      while not IfMatch(CSTII_end) do NextToken;
      Match(CSTI_SemiColon);
